...
| **Commit gate** | `/canonify-commit` - audit diff against the same canons; also (planned, with Doctor) refresh `verified` SHAs and flag canons whose referenced files the diff touched (incremental staleness) | Audit Done (ceosite); staleness duty Planned |
...
| **Framework <-> content split** | portable skills vs per-repo `CANONIFY.md` + docs | Done - all 6 gates generic + namespaced in `canonify/skills/`; content (`CANONIFY.md` + docs) stays per-repo |
...
| Generic skills lifted into `canonify/skills/` (no project specifics) | Done - all 6 gates (plan/build/commit/create-canon/kickoff/doctor) generic + namespaced |
...
Canonify ships v1.0 when: all gates exist (plan/build/commit, create-canon, kickoff, doctor); the OSS
